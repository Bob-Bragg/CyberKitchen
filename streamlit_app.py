...
from ck_ops import OPS, Operation, to_bytes, try_decode_utf8, bytes_to_hex, clamp_bytes, magic_detect
            ...
            for pname, default in schema.items():
                with cols2[slot % 3]:
                    if isinstance(default, list):
                        step.params[pname] = st.selectbox(pname, options=default, index=default.index(default[0]) if default else 0, key=f"{pname}_{idx}")
